...
- **FAILURE MODE — hybrid (attribute + loose-MText) revision table (source defect, handle MANUALLY):** some drawings author only the EARLY revision rows as block attributes and later rows as **standalone MText drawn loosely into the layout at half-row-offset Y** (E-PLN-003 `Coleamablly`: rows A/B = attributes at y14.7/21.0; rows C "MATCH TO AUXILIARY BLOCK DIAGRAM" / D "ISSUED FOR CONSTRUCTION" = loose MText at y30.6/36.9). `_update_title_block` only sees ATTRIBUTES → counts A/B occupied, writes AS BUILT into attribute row3 (y27.3), which then **physically collides with the loose C MText (y30.6)** and never rolls the loose C/D rows. Diagnose by dumping STANDALONE Text/MText in the TB area (not just `GetAttributes()`) — the loose rows show up there. This is a **source-authoring defect**: the rolling logic is correct for proper attribute tables (verified on E-CFG-001 etc.). Fix = correct the SOURCE (re-enter the loose C/D rows as real `{n}` attributes), then re-convert — clean.
- **AUTO-DETECTION (so the class is caught with zero manual re-diagnosis): `_count_loose_rev_dates(space, attrs)`** runs at the TOP of `_update_title_block` (before any fallback text, so it never miscounts the bot's own output) and counts STANDALONE date-text (`\d{1,2}/\d{1,2}/\d{2,4}`) sitting in the attribute revision band. >0 → appends a `需人工检查` tb-QA WARN (`标题栏含 N 处松散文字版本行…`). **Gold-safe by construction**: (a) it only ADDS a non-blocking WARN — never fails/retries a shippable PDF; (b) it lives in the conversion path, which `golden/test_qa_regression.py` never runs (that gate only QA's existing PDFs), so the locked verdicts are untouched; (c) a clean attribute table has NO standalone date in the band → 0; (d) ModelSpace iteration is capped (`Count>4000` → skip). Validated 2026-06-09: fires on E-PLN-003 (S01/S02=2, S03=1, matching COM truth), does NOT fire on controls E-CFG-001 / E-BLD-003. It DETECTS only — it does NOT auto-fix (no risky loose-MText parser/rewriter in the shared path); remediation stays correct-the-source / per-drawing manual like E-BLD-001.
...
- **Multi-title doc-id → adjudicated by the client *Deliverables List* (the TITLE authority), NOT wholesale-deferred** (`register_membership.py`, the cross-track SSOT; AB gate landed 2c76466): the doc-id is the join key, the register maps it to its canonical Doc Name(s). A doc-id the register lists under ONE title → files that only drifted (`SLD` vs `Single Line Diagram`) auto-reconcile + version together (NO human step). A doc-id listed under SEVERAL titles (`CA-001` = Trench Alignment + Cable Route GA) → genuinely several drawings, each versioned independently. A title matching NO register name → flagged `stray` (never merged/renamed — guards the normalize-Rule-3 data-loss trap). Register absent for the doc-id → conservative wholesale defer (old behavior). Same-mtime collisions still flagged. Reconcile test = fuzzy ≥0.85 OR acronym-of OR identity-token containment (Jaccard ≥0.6 / subset).
- **Same oracle now covers all FOUR version tracks** (c08ba01) via `register_membership.py`: AS BUILT deliverable PDFs (`supersede_ab_deliverables`), IFR/IFC deliverable PDFs (`VersionManager` — fixes the UNDER-merge: re-worded revs now share a register-canonical key so the old one supersedes), and Native DWG (`NativeVersionManager` — fixes the DATA-LOSS direction: a doc-id folder holding two real drawings is split by register title so each keeps its own newest; the multi-drawing case also skips the folder-name standardize-rename to avoid same-name overwrite). Applied in BOTH `ifr_automation_v10.py` (bot's production path) and standalone `version_manager_v5.py`. SOFT + zero-regression: no Deliverables List → register empty → grouping byte-identical to the old base_name/ext behavior (the common case). The engine's AB `_ab_title_*` helpers are now thin delegations to `register_membership.py` (0ceb0e7) — one SSOT, no inlined duplicate.
- **Filename-format standardization** (`register_membership.normalize_filename_format` — IDENTITY-PRESERVING: case/space/underscore/`Rev` only, description words untouched, so it can never collapse two drawings). Each collision gate only ever **PREVIEWS** it (`_preview_name_format`, prints `[格式预览] old -> new`, 8023b3e). The **execute side** is a separate, deliberate pipeline step `standardize_filenames.py` — DRY-RUN by default, `--apply` required to touch disk, `-r`/`--ext` optional. COLLISION GUARD: two sources normalizing to one name, or a pre-existing different target, are skipped (never overwrite/merge); case-only renames route through a temp name (WindowsPath compares case-insensitively). Run per-project, by a human, with explicit `--apply` — never auto-run on client folders.
- **LMS 2026-06-26**: 6 reversible moves applied (CA-011/EA-300/EA-301/ES-010/GA-001/GA-051 Rev1 → `Superseded/`); 3 multi-title (CA-001/EL-001/EL-002) previously skipped for manual review — now auto-adjudicated by the register oracle when the Deliverables List is present.
...
### Deliverable Title Capacity ↔ Capacity SSOT (cross-project drift check)
- `DeliverableManager.cross_check()` runs `_reconcile_title_energy(ws)` for **every** project: reads the DLV title capacity (C1, e.g. `10.03MWh` / `4.98MVA`) and compares it to the SAME physical quantity in the **capacity 真源**. **Read-only — never overwrites the DLV.**
- **Capacity SSOT = 真源, arrow corrected 2026-07-07**: authority chain is **`ACE_Project_Input` (真源, owns `Power`+`Energy (MWh)` cols) → spine `projects.db` → `ACE_Portfolio` (legacy fallback only)** — via `portfolio_energy.lookup_capacity()`. ACE_Portfolio is a **downstream view**, NOT the source (earlier code had the arrow backwards — looking *up* to Portfolio; that's the bug this fixed). Waterloo's `10.03MWh` truly lives in the spine (Sunwoda equipment capture) and is now seeded into the Input真源, so the check returns **ok** (was skip-on-blank).
...
- **Dimension-aware**: energy (MWh/kWh) and power (MVA/MW/kVA) are different quantities → C1's own unit picks which quantity to compare (Energy vs Power); kVA↔MVA / kWh↔MWh normalized, MW≈MVA at PF≈1. Never cross-fill quantities (a PV project's C1 is MVA, a BESS project's is MWh).
- **Project match is exact** (type-suffix stripped): `Waterloo`→`waterloo` matches `Waterloo`, but **never collapses `Waterloo #2`→`Waterloo`** (would grab a different project's number). Project name = the project **folder** name (`self.project_path.name`) → nothing hardcoded.
- **Gate**: match within ε (0.5% / 0.01) → silent OK; 真源 has no value for that project/quantity, or the title has no capacity token → silent **skip** (no false warn); **material mismatch → WARN `[需人工检查]`** in run log + pipeline summary, and **NEVER auto-overwrite** (no provenance to decide who's newer). Auto-resolution would need a provenance/LWW event-table — deliberately deferred, not owed as debt.
- **Seeding / maintenance**: `python portfolio_energy.py seed [--dry]` backfills the Input真源 capacity cols (Portfolio value first, else spine) — idempotent, fills blanks only, never clobbers a hand-edited Input value. Pending next step (not yet wired): reverse projection `Input → ACE_Portfolio` write-back so the exec看板 reflects真源 (gated on user nod — it edits the hand-maintained exec file).
...
- **Overlap trigger MUST include LINEWORK** (not just words/cells): besides (a) foreign word-in-box and (b) foreign cell rect, the trigger also flags (c) foreign LINEWORK — a bare drawing/geometry stroke crossing a box. A wiring schematic / single-line diagram is mostly lines with few words and no table cells, so a words+cells-only test FALSE-PASSES a stamp on live linework (LMS EL-002 Protection Key Diagram). Never narrow the trigger back to words/cells only.
...
- **LMS bespoke path now gated**: `asbuilt_revclean_lms.py` (7 LMS drawings GA-001/CA-001/CA-011/EA-013/EA-300/EA-301/EL-002) previously had ZERO stamp-overlap QA. `qa_stamp_overlap()` now runs after `publish_pdf`, REUSING this detector via `AsBuiltManager.__new__` (bypasses the COM-connecting `__init__`; import pulls in win32com as a library only, does not drive AutoCAD). Read-only: on overlap it prints `[需人工检查] <file> page N: 印章压图面内容` and flags the run (`run()` returns not-clean) — it NEVER alters/deletes the PDF.
