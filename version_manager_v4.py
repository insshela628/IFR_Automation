#!/usr/bin/env python3
"""
Version Manager - 旧版本文件管理工具 v4.0 (STANDALONE FALLBACK)
...
Manages file versions across:
  ...
  Mode 2 (Native): Native + Reports + Schedule folders — all working files
    ...
    - Keeps newest IFR + newest IFC per folder
...
class NativeVersionManager:
    ...
    def process_folder(self, folder: Path) -> NativeFolderResult:
        ...
        # OTHER: move to SS (unless it's the only file in folder)
        for other in other_files:
            if len(files) > 1:
                self._plan_move(result, other, ss_folder, "unversioned/legacy")

        # Rename kept files to standard format
            ...
            for _, result in group_list:
                ...
                if result.kept_ifr:
                    print(f"      [保留 IFR] {result.kept_ifr.filename}"
                          f"  ({result.kept_ifr.rev_display}, "
                          f"{result.kept_ifr.mtime.strftime('%Y-%m-%d %H:%M')})")
                if result.kept_ifc:
                    print(f"      [保留 IFC] {result.kept_ifc.filename}"
                          f"  ({result.kept_ifc.rev_display}, "
                          f"{result.kept_ifc.mtime.strftime('%Y-%m-%d %H:%M')})")
